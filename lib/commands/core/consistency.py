...
def check_history_consistecy() -> bool:
    ...
    doc_file_bodies: Set =  {
        os.path.splitext(file)[0] for file in os.listdir(doc_dir)
        if os.path.isfile(os.path.join(doc_dir, file)) and not re.match(r".", file)
        }
    hist_file_bodies: Set =  {
        os.path.splitext(file)[0] for file in os.listdir(hist_dir)
        if os.path.isfile(os.path.join(hist_dir, file)) and not re.match(r".", file)
        }
